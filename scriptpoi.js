...
function isQueenTrapped(color) {
  ...
  // On amorce depuis les cases de bord SAUF la case de la Reine elle-même.
  // On traverse tout sauf les dépouilles (murs) et la case de la Reine (destination).
  // Si on atteint la case de la Reine → elle est connectée à l'extérieur → pas piégée.
  const qR = chef.r, qC = chef.c;
  const visited = new Set();
  const queue = [];

  for (let i = 0; i < 9; i++) {
    for (const [r,c] of [[0,i],[8,i],[i,0],[i,8]]) {
      if (r === qR && c === qC) continue; // ne pas amorcer depuis la case de la Reine
      const key = `${r},${c}`;
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) continue;
      if (nr === qR && nc === qC) {
        console.log('[ENCERCLEMENT] '+color+' : Reine en ('+qR+','+qC+') atteignable → pas piégée');
        return false; // on a atteint la Reine depuis l'extérieur
      }
  ...
  console.log('[ENCERCLEMENT] '+color+' : Reine en ('+qR+','+qC+') NON atteignable → ENCERCLÉE');
  return true;
}
