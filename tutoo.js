...
const SYM = {
  queen:'reine_guepe_512.webp', spider:'araignee_512.webp', fly:'mouche_512.webp', beetle:'scarabee_512.webp', lady:'coccinelle_512.webp', ant:'fourmi_512.webp',
};
...
const SLIDES = [
  ...
  /* 1 — GUÊPE REINE */
  {
    type: 'piece', sym: 'reine_guepe_512.webp', name: `La Guêpe Reine`,
  ...
  /* 2 — ARAIGNÉE */
  {
    type: 'piece', sym: 'araignee_512.webp', name: `L'Araignée Assassine`,
    ...
    tags: [{cls:'kill',txt:`Prend la place de la cible`},{cls:'spec',txt:`Dépouille 💀 reste au départ`}],
    boardSize: 5,
  ...
  /* 3 — MOUCHE */
  {
    type: 'piece', sym: 'mouche_512.webp', name: `La Mouche Journaliste`,
  ...
  /* 4 — SCARABÉE */
  {
    type: 'piece', sym: 'scarabee_512.webp', name: `Le Scarabée Déplaceur de Dépouilles`,
    title: `Le Scarabée — Maître des dépouilles`,
    desc: `Se déplace en <b>dame aux échecs</b>.
           Il va chercher une dépouille 💀 dans sa ligne de vision et la <span class="th-gold">redépose sur n'importe quelle case libre</span>.
           <b>Super Pouvoir</b> : peut choisir <span class="th-special">n'importe quelle dépouille</span> du plateau, sans restriction de ligne de vision.
           <span class="th-red">Sans lui, votre Reine peut être encerclée et piégée.</span>`,
    tags: [{cls:'move',txt:`Dame aux échecs`},{cls:'spec',txt:`Déplace les dépouilles 💀`},{cls:'warn',txt:`SP : Déplacement libre 🪲`}],
    boardSize: 5,
  ...
  /* 5 — COCCINELLE */
  {
    type: 'piece', sym: 'coccinelle_512.webp', name: `La Coccinelle Déplaceuse de Vivant`,
    ...
    desc: `Se déplace en <b>dame aux échecs</b> et atterrit <b>sur une pièce ennemie vivante</b>.
           ...
           <b>Super Pouvoir</b> : peut <span class="th-special">ressusciter n'importe quelle dépouille</span> — elle rejoint la colonie de la Coccinelle (et garde son SP si elle en avait un).`,
    tags: [{cls:'spec',txt:`Va sur la pièce ennemie`},{cls:'move',txt:`Redépose n'importe où`},{cls:'warn',txt:`SP : Résurrection 💀✨`}],
    boardSize: 5,
  ...
  /* 6 — FOURMI */
  {
    type: 'piece', sym: 'fourmi_512.webp', name: `Les Fourmis Soldates`,
  ...
  /* 8 — NID SACRÉ */
  {
    ...
    stakes: [
      { ico:'👑', txt:`<b>Accès réservé</b> — Seule la Reine peut <b>s'y arrêter librement</b>. Toutes les autres pièces le traversent sans s'y arrêter.` },
      { ico:'⚔️', txt:`<b>Tuer sur le Nid</b> — N'importe quelle pièce (sauf les Fourmis) peut aller tuer la Reine ennemie sur le Nid. Après ce kill, la pièce <b>doit quitter le Nid</b> avec un <b>coup bonus</b> — ce coup permet d'y amener sa propre Reine !` },
  ...
  /* 9 — SUPER POUVOIRS */
  {
    ...
    desc: `Option activable avant la partie — <b>activée par défaut</b> :`,
    stakes: [
      { ico:'⚙️', txt:`<b>Optionnel</b> — Les super pouvoirs sont <b>activés par défaut</b>. Vous pouvez les désactiver dans les options avant de lancer une partie.` },
      { ico:'✨', txt:`<b>Cases aura</b> — Quand activés, des cases violettes pulsantes apparaissent tous les 10 coups. La première pièce à s'y poser reçoit un super pouvoir unique.` },
      { ico:'👑', txt:`<b>Invincibilité (Reine)</b> — La Reine devient temporairement invincible pendant <b>4 de ses propres tours</b>. Aucune pièce ne peut la tuer durant ce temps.` },
      { ico:'🐜', txt:`<b>Portée Reine (Fourmi)</b> — La Fourmi se déplace comme une Reine aux échecs : portée illimitée dans toutes les directions.` },
      { ico:'🐞', txt:`<b>Résurrection (Coccinelle)</b> — La Coccinelle choisit <b>n'importe quelle dépouille</b> sur le plateau et la ressuscite. Elle rejoint la colonie de la Coccinelle (et garde son SP si elle en avait un).` },
      { ico:'🪲', txt:`<b>Déplacement libre (Scarabée)</b> — Le Scarabée peut choisir <b>n'importe quelle dépouille</b> sur le plateau (pas seulement celles dans sa ligne de vision) et la replacer sur n'importe quelle case vide.` },
      { ico:'🪰', txt:`<b>Zone totale (Mouche)</b> — La Mouche attaque dans les <b>8 directions</b> simultanément au lieu de choisir entre ortho et diagonal.` },
    ],
  },
...
function buildQueenAnim() {
  return {
    piece:{r:2,c:2,color:'yellow',sym:'reine_guepe_512.webp'},
    enemy:{r:0,c:4,color:'red',sym:'fourmi_512.webp'},  // fourmi ennemie
    frames:[
      ...
      {delay:1500, label:{text:'Dépouille repositionnée sur n\'importe quelle case libre.', type:'corpse', dur:2000}, cells:[], piece:null, placeCorpse:{r:3,c:1,sym:'fourmi_512.webp'}},
      // Reset
...
function buildSpiderAnim() {
  return {
    piece:{r:2,c:1,color:'yellow',sym:'araignee_512.webp'},
    enemy:{r:2,c:4,color:'blue',sym:'coccinelle_512.webp'},  // coccinelle ennemie
    frames:[
      // Montre portée dame
      {delay:500, label:{text:'Parcourt lignes et diagonales sans limite de portée.', type:'move'}, cells:[
      ...
      {delay:1800, label:{text:'Élimine la cible puis occupe immédiatement sa case.', type:'kill'}, cells:[], piece:{r:2,c:4}, killEnemy:true, corpseAt:{r:2,c:1,sym:'coccinelle_512.webp'}},
...
function buildFlyAnim() {
  ...
  return {
    piece:{r:4,c:4,color:'yellow',sym:'mouche_512.webp'},
    enemies:[
      {r:1,c:2,color:'red',   sym:'fourmi_512.webp'}, // ortho haut
      {r:2,c:1,color:'blue',  sym:'araignee_512.webp'}, // ortho gauche
      {r:2,c:3,color:'green', sym:'reine_guepe_512.webp'}, // ortho droite
      {r:3,c:2,color:'red',   sym:'scarabee_512.webp'}, // ortho bas
    ],
    enemiesDiag:[
      {r:1,c:1,color:'red',   sym:'fourmi_512.webp'},
      {r:1,c:3,color:'blue',  sym:'araignee_512.webp'},
      {r:3,c:1,color:'green', sym:'reine_guepe_512.webp'},
      {r:3,c:3,color:'red',   sym:'scarabee_512.webp'},
    ],
    flyPhase: true,
    frames:[
...
function buildBeetleAnim() {
  ...
  return {
    piece:{r:2,c:0,color:'yellow',sym:'scarabee_512.webp'},
    corpse:{r:0,c:2,sym:'fourmi_512.webp'}, // dépouille fourmi — accessible en diag ↗
    frames:[
...
function buildLadybugAnim() {
  return {
    piece:{r:2,c:1,color:'yellow',sym:'coccinelle_512.webp'},
    enemy:{r:2,c:3,color:'red',sym:'mouche_512.webp'}, // mouche ennemie
    frames:[
      // Montre portée dame
      {delay:500, label:{text:'Parcourt lignes et diagonales sans limite de portée.', type:'move'}, cells:[
        {r:0,c:1,cls:'t-move'},{r:1,c:1,cls:'t-move'},{r:3,c:1,cls:'t-move'},{r:4,c:1,cls:'t-move'},
...
function buildAntAnim() {
  ...
  return {
    piece:{r:3,c:3,color:'yellow',sym:'fourmi_512.webp'},
    enemy:{r:1,c:3,color:'blue',sym:'scarabee_512.webp'},
    frames:[
      ...
      {delay:1800, label:{text:'Dépouille repositionnée.', type:'corpse', dur:1800}, cells:[], piece:null, placeCorpse:{r:5,c:0,sym:'scarabee_512.webp'}},
      // Reset
...
function _buildContent(el, slide, idx) {
  if(slide.type==='intro') {
    ...
    el.innerHTML=`
      <div class="tuto-intro-icon"><img src="reine_guepe_512.webp" alt="Reine Guêpe" style="width:3rem;height:3rem;object-fit:contain;"></div>
...
function _placePiece(board, state) {
  ...
  el.innerHTML=`<img src="${state.piece.sym||'reine_guepe_512.webp'}" alt="" style="width:110%;height:110%;object-fit:contain;position:absolute;top:-5%;left:-5%;pointer-events:none;">`; 
}
...
function _placeEnemy(board, enemy, eid, diagIdx) {
  ...
  el.style.left=left; el.style.top=top; el.style.width=size; el.style.height=size;
  el.innerHTML=`<img src="${enemy.sym||'fourmi_512.webp'}" alt="" style="width:110%;height:110%;object-fit:contain;position:absolute;top:-5%;left:-5%;pointer-events:none;">`;
  board.appendChild(el);
}
...
function _placeCorpse(board, pos, n) {
  ...
  el.style.left=left; el.style.top=top; el.style.width=size; el.style.height=size;
  el.innerHTML=`<img src="${pos.sym||'fourmi_512.webp'}" alt="" style="width:110%;height:110%;object-fit:contain;position:absolute;top:-5%;left:-5%;pointer-events:none;">`;
  board.appendChild(el);
}
