...
function openReplayModal() {
  const el = document.getElementById('replay-log-body');
  if (!el) return;
  if (!_moveLog.length) {
    el.innerHTML = '<div style="font-size:.82rem;color:var(--text-muted);font-style:italic;text-align:center;padding:12px 0;">Aucun coup joué</div>';
  } else {
    el.innerHTML = _moveLog.map((e, i) => `
      <div style="display:flex;align-items:baseline;gap:8px;padding:7px 10px;border-radius:8px;background:rgba(255,255,255,.03);border:1px solid var(--border);">
        <span style="width:8px;height:8px;border-radius:50%;background:${e.color};flex-shrink:0;margin-top:3px;"></span>
        <span style="font-size:.78rem;line-height:1.35;color:rgba(224,220,255,.88);">${e.text}</span>
        <span style="font-size:.68rem;color:var(--text-muted);margin-left:auto;flex-shrink:0;">T${_gameTurns - i}</span>
      </div>`).join('');
  }
  showModal('replay');
}

function replayPrev() {} // conservé pour compatibilité
function replayNext() {} // conservé pour compatibilité
