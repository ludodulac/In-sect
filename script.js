...
// Lance le tirage du pacte (animé sous l'échiquier, pas de modale)
function launchPactDraw() {
  if (G.mode1) return;
  const alive = G.order.filter(c => G.players[c] && G.players[c].alive);
  if (alive.length < 3) return;

  const area = document.getElementById('pact-draw-area');
  if (!area) return;

  // Tirage final au sort
  const shuffled = [...alive].sort(() => Math.random() - .5);
  const [c1, c2] = shuffled;

  // Animation de tambour : faire défiler les noms rapidement puis ralentir
  const allColors = [...alive];
  let tick = 0;
  const totalTicks = 22;
  let cur1 = 0, cur2 = 1;

  area.innerHTML = `<div class="pact-wheel">
    <span style="font-size:.7rem;color:var(--text-muted);">Tirage…</span>
    <div class="pact-slot" id="pslot1"></div>
    <span style="color:var(--text-muted);font-size:.8rem;">&amp;</span>
    <div class="pact-slot" id="pslot2"></div>
  </div>`;

  function drumStep() {
    const progress = tick / totalTicks;
    const delay = 60 + Math.pow(progress, 2) * 340; // accélère → ralentit

    // Afficher deux couleurs différentes au hasard
    const s1 = allColors[tick % allColors.length];
    const s2 = allColors[(tick + 1) % allColors.length] === s1
      ? allColors[(tick + 2) % allColors.length]
      : allColors[(tick + 1) % allColors.length];

    const slot1 = document.getElementById('pslot1');
    const slot2 = document.getElementById('pslot2');
    if (slot1) { slot1.textContent = CNAME_SHORT[s1] || '?'; slot1.style.color = CCSS[s1] || '#fff'; slot1.style.borderColor = (CCSS[s1]||'#fff') + '88'; }
    if (slot2) { slot2.textContent = CNAME_SHORT[s2] || '?'; slot2.style.color = CCSS[s2] || '#fff'; slot2.style.borderColor = (CCSS[s2]||'#fff') + '88'; }

    tick++;
    if (tick < totalTicks) {
      setTimeout(drumStep, delay);
    } else {
      // Résultat final
      setTimeout(() => showPactResult(c1, c2), 200);
    }
  }
  drumStep();
}
...
function showPactResult(c1, c2) {
  ...
  area.innerHTML = `<div class="pact-result">
    <span class="pact-dot" style="background:${col1};box-shadow:0 0 6px ${col1};"></span>
    <span style="color:${col1};font-weight:900;">${CNAME_SHORT[c1]}</span>
    <span style="color:var(--text-muted);font-size:.75rem;">et</span>
    <span class="pact-dot" style="background:${col2};box-shadow:0 0 6px ${col2};"></span>
    <span style="color:${col2};font-weight:900;">${CNAME_SHORT[c2]}</span>
  </div>
  <div style="font-size:.68rem;color:var(--text-muted);text-align:center;line-height:1.5;">Ne peuvent se causer aucun dégât.<br>Prend fin si un outsider est éliminé.</div>`;
}
...
function togglePactOption() {
  ...
  updateToggleUI();
  if (!_optPact) {
    // Désactiver : effacer le pacte et afficher "inactif"
    G.pact = null;
    G.pactProposed = false;
    const area = document.getElementById('pact-draw-area');
    if (area) area.innerHTML = `<div class="pact-inactive">Pacte désactivé</div>`;
    toast('🤝 Pacte désactivé');
  } else {
    // Réactiver : relancer un tirage
    G.pact = null;
    G.pactProposed = false;
    launchPactDraw();
    toast('🤝 Nouveau tirage du pacte…');
  }
}
...
// Vérifie si le pacte doit prendre fin
function checkPactEnd() {
  ...
  if (outsiders.length === 0) {
    G.pact = null;
    const area = document.getElementById('pact-draw-area');
    if (area) area.innerHTML = `<div class="pact-inactive">Pacte terminé — outsider éliminé</div>`;
    toast('🤝 Le pacte est terminé');
  }
}
...
function initPactIfNeeded() {
  ...
  const area = document.getElementById('pact-draw-area');
  if (area) area.innerHTML = `<div class="pact-inactive">Tirage en cours…</div>`;
...
function launchSPAnimation() {
  ...
  // Afficher message au-dessus du plateau
  const pm = document.getElementById('phase-msg');
  if (pm) { pm.innerHTML = `<span class="sp-spin-msg">✨ Spirale du Super Pouvoir ✨</span>`; pm.classList.add('visible'); }
  ...
  function cleanup() {
    if (pm) { pm.innerHTML = ''; pm.classList.remove('visible'); }
