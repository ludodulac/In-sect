...
function isQueenTrapped(color) {
  ...
  // Si un Scarabée allié est vivant → pas piégée (peut déplacer les dépouilles)
  const hasNecro = pl.pieces.some(p=>!p.dead&&p.type==='necromobile'&&p.color===color);
  if (hasNecro) { console.log('[ENCERCLEMENT] '+color+' : Scarabée vivant → pas piégée'); return false; }

  // Flood-fill depuis l'EXTÉRIEUR : on part de toutes les cases du bord (qui ne sont pas des dépouilles)
  // et on envahit l'intérieur en évitant les dépouilles (seuls vrais murs).
  // Si on atteint la case de la Reine → elle communique avec l'extérieur → pas encerclée.
  // Si le fill s'arrête sans jamais atteindre la Reine → encerclée.
  const visited = new Set();
  const queue = [];

  // Amorcer avec toutes les cases des 4 bords
  for (let i = 0; i < 9; i++) {
    for (const [r,c] of [[0,i],[8,i],[i,0],[i,8]]) {
      const key = `${r},${c}`;
      if (visited.has(key)) continue;
      const cell = G.board[r][c];
      if (cell && cell.dead) continue; // dépouille sur le bord = mur, ne pas entrer
      visited.add(key);
      queue.push([r,c]);
    }
  }
  ...
  while (queue.length) {
    ...
    if (r === chef.r && c === chef.c) {
      console.log('[ENCERCLEMENT] '+color+' : Reine en ('+chef.r+','+chef.c+') atteignable depuis le bord → pas piégée');
      return false;
    }
    for (const [dr,dc] of DIRS8) {
  ...
  console.log('[ENCERCLEMENT] '+color+' : Reine en ('+chef.r+','+chef.c+') NON atteignable → ENCERCLÉE !');
  return true;
}
...
function showSPResult(piece, spType) {
  ...
  // Si la pièce appartient à l'IA → fermer automatiquement après 1.8s
  const isHumanPiece = piece.color === G.human;
  if (!isHumanPiece) {
    setTimeout(() => closeSPResult(), 1800);
  }
}
