...
function updatePieceInfo(piece) {
  ...
  // Pour la reine : image spéciale guêpe.png (grande illustration)
  const displayImg = piece.type === 'chef' ? 'guepe.png' : getSYM(piece.type, piece.color);
  pi.innerHTML = `
    <div class="pi-img-wrap">
      <img src="${displayImg}" alt="${PNAME[piece.type]}" class="pi-insect-big">
    </div>
    <div class="pi-info">
      <div class="pin" style="color:${CCSS[piece.color]}">${PNAME[piece.type]}</div>
      <div class="pid">${PDESC[piece.type]}</div>
    </div>`;
