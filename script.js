...
function drawReplayBoard(snapshot) {
  ...
  const CBG  = { yellow:'#6B4F00', red:'#5B0000', green:'#0D3B00', blue:'#0A2A5E' };
  const CBRD = { yellow:'#FFD700', red:'#FF3050', green:'#4CAF50', blue:'#42A5F5' };

  // Damier
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      ctx.fillStyle = (r + c) % 2 === 0 ? '#1a1a2e' : '#22244a';
      ctx.fillRect(c * cs, r * cs, cs, cs);
    }
  }

  // Nid sacré
  ctx.fillStyle = 'rgba(212,160,23,.2)';
  ctx.fillRect(4*cs, 4*cs, cs, cs);
  ctx.strokeStyle = 'rgba(212,160,23,.5)';
  ctx.lineWidth = 1.5;
  ctx.strokeRect(4*cs+1, 4*cs+1, cs-2, cs-2);

  // Pièces
  const rad = cs * 0.42;
  for (const p of snapshot.pieces) {
    const x = p.c * cs + cs/2, y = p.r * cs + cs/2;
    ctx.globalAlpha = p.dead ? 0.4 : 1;
    // Cercle fond
    ctx.beginPath();
    ctx.arc(x, y, rad, 0, Math.PI*2);
    ctx.fillStyle = p.dead ? '#2a2a2a' : (CBG[p.color] || '#333');
    ctx.fill();
    ctx.strokeStyle = p.dead ? '#555' : (CBRD[p.color] || '#aaa');
    ctx.lineWidth = p.dead ? 1 : 2;
    ctx.stroke();
    // Image webp
    const imgSrc = SYM[p.type];
    if (imgSrc) {
      const img = getReplayImg(imgSrc);
      const iSize = rad * 1.45;
      if (img.complete && img.naturalWidth > 0) {
        ctx.save();
        ctx.beginPath();
        ctx.arc(x, y, rad - 1, 0, Math.PI*2);
        ctx.clip();
        if (p.dead) ctx.filter = 'grayscale(100%)';
