...
function startGame() {
  ...
  const pactRow = document.getElementById('gtog-pact-row');
...
function updateTurnUI() {
  ...
  // Info panel : tour à gauche
  const dot = document.getElementById('info-turn-dot');
  const txt = document.getElementById('info-turn-txt');
  if (dot) { dot.style.background = CCSS[c]; dot.style.color = CCSS[c]; }
  if (txt) { txt.textContent = `Tour de ${CNAME[c]}${pl.human?' (Vous)':''}`; txt.style.color = CCSS[c]; }
  // Info panel border
  const ip = document.getElementById('info-panel');
  if (ip) { ip.style.borderColor = CCSS[c]+'44'; ip.style.boxShadow = `0 0 10px ${CGLOW[c]}`; }
  // Message de phase à droite
  const phases = {
    'place-corpse':    '💀 Placez la dépouille',
    'place-dipl':      '🐞 Déposez la pièce',
    'place-necro':     '🪲 Déposez la dépouille',
    'reporter-choose': '🪰 Ortho 🔴 ou Diag 🟠 ?',
  };
  const msg = phases[G.phase] || '';
  const mr = document.getElementById('info-msg-right');
  if (mr) mr.textContent = msg;
}

function updatePieceInfo(piece) {
  const detail = document.getElementById('info-detail');
  const dname  = document.getElementById('info-detail-name');
  const ddesc  = document.getElementById('info-detail-desc');
  if (!detail) return;
  detail.style.display = '';
  const sp = G.spPieces && G.spPieces[piece.id];
  const fullName = (PFULL[piece.type] || PNAME[piece.type]);
  const spTag = sp ? ` <span style="color:#C080FF;font-size:.7rem;">✨ ${SP_DESC[sp.type].split('!')[0]}${sp.type==='invincible'?' ('+sp.turns+' coups)':''}</span>` : '';
  if (dname) { dname.innerHTML = `<span style="color:${CCSS[piece.color]}">${fullName}</span>${spTag}`; }
  if (ddesc)  { ddesc.innerHTML = PDESC[piece.type]; }
}
...
function showInfoText(name, desc, color) {
  ...
  if (!detail) return;
  detail.style.display = '';
  if (dname) dname.innerHTML = `<span style="color:${color||'var(--gold)'}">${name}</span>`;
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
function handlePactToggleClick(e) {
...
function launchPactDraw() {
  ...
  const slot = document.getElementById('pact-inline');
  if (!slot) return;
  ...
  function drumStep() {
    ...
    const s2 = allColors[(tick + 2) % allColors.length] !== s1
      ? allColors[(tick + 2) % allColors.length]
      : allColors[(tick + 1) % allColors.length];
    slot.innerHTML =
      `<span class="pact-dot" style="background:${CCSS[s1]};"></span>` +
      `<span class="pact-slot" style="color:${CCSS[s1]}">${CNAME_SHORT[s1]}</span>` +
      `<span class="pact-sep">&amp;</span>` +
      `<span class="pact-dot" style="background:${CCSS[s2]};"></span>` +
      `<span class="pact-slot" style="color:${CCSS[s2]}">${CNAME_SHORT[s2]}</span>`;
    tick++;
...
function showPactResult(c1, c2) {
  ...
  slot.innerHTML =
    `<span class="pact-dot" style="background:${col1};box-shadow:0 0 4px ${col1};"></span>` +
    `<span class="pact-slot" style="color:${col1};border-color:${col1}44">${CNAME_SHORT[c1]}</span>` +
    `<span class="pact-sep">&amp;</span>` +
    `<span class="pact-dot" style="background:${col2};box-shadow:0 0 4px ${col2};"></span>` +
    `<span class="pact-slot" style="color:${col2};border-color:${col2}44">${CNAME_SHORT[c2]}</span>`;
}
...
function launchSPAnimation() {
  ...
  // Afficher message dans info-msg-right
  const mr = document.getElementById('info-msg-right');
  if (mr) mr.innerHTML = `<span class="sp-spin-msg">✨ Spirale du Super Pouvoir ✨</span>`;
  ...
  function cleanup() {
    const mr2 = document.getElementById('info-msg-right');
    if (mr2) mr2.innerHTML = '';
