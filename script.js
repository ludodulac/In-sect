...
function updateStatusBar() {
  ...
  // Score victoires/défaites — toujours visible, par mode+difficulté sélectionné
  ...
  const modeLabel= document.getElementById('score-mode-label');
  if (scoreBar) {
    ...
    const lvlLabel = ['','Basique','Avancé','Expert'][_aiLevel] || '';
    if (modeLabel) modeLabel.textContent = `${_mode} IA · ${lvlLabel}`;
...
function snapshotBoard() {
  // Capture légère : position + état de chaque pièce vivante ou dépouille sur le board
  const pieces = [];
  for (const c of G.order.concat(Object.keys(G.players).filter(c=>!G.order.includes(c)))) {
    const pl = G.players[c]; if (!pl) continue;
    for (const p of pl.pieces) {
      if (!inB(p.r, p.c)) continue;
      pieces.push({ id:p.id, color:p.color, type:p.type, r:p.r, c:p.c, dead:p.dead });
    }
  }
  return { pieces, turn: G.turn };
}

function logMove(entry) {
  entry.snapshot = snapshotBoard(); // snapshot du plateau à ce moment
...
/* ── Replay des derniers coups ── */
let _replayIdx = 0; // 0 = le plus récent

function openReplayModal() {
  if (!_moveLog.length) { toast('Aucun coup enregistré.'); return; }
  _replayIdx = 0;
  renderReplayFrame();
  showModal('replay');
}
...
function renderReplayFrame() {
  const entry = _moveLog[_replayIdx];
  if (!entry || !entry.snapshot) return;

  // Texte du coup
  const txtEl = document.getElementById('replay-move-text');
  if (txtEl) txtEl.innerHTML = entry.text || '';

  // Compteur
  const cntEl = document.getElementById('replay-counter');
  if (cntEl) cntEl.textContent = `${_moveLog.length - _replayIdx} / ${_moveLog.length}`;

  // Navigation
  document.getElementById('replay-prev').disabled = (_replayIdx >= _moveLog.length - 1);
  document.getElementById('replay-next').disabled = (_replayIdx <= 0);

  // Dessiner le board (attendre que les images soient chargées)
  const canvas = document.getElementById('replay-canvas');
  if (canvas) {
    // Ajuster la taille du canvas à son conteneur
    const wrap = canvas.parentElement;
    if (wrap) {
      const sz = Math.min(wrap.clientWidth, wrap.clientHeight, 378);
      canvas.width = sz; canvas.height = sz;
    }
  }
  drawReplayBoard(entry.snapshot);

  // Animation prise si c'est un kill
  if (entry.isKill) {
    showReplayKillAnim(entry.snapshot, canvas);
  }
}

function showReplayKillAnim(snapshot, canvas) {
  const animEl = document.getElementById('replay-kill-anim');
  if (!animEl || !canvas) return;
  // Trouver la pièce tuée (la première dépouille dans le snapshot)
  const killed = snapshot.pieces.find(p => p.dead);
  if (!killed) return;
  const cs = canvas.width / 9;
  const rect = canvas.getBoundingClientRect();
  const wrapRect = canvas.parentElement.getBoundingClientRect();
  const x = rect.left - wrapRect.left + killed.c * cs + cs/2;
  const y = rect.top  - wrapRect.top  + killed.r * cs + cs/2;
  animEl.style.left = `${x - 16}px`;
  animEl.style.top  = `${y - 16}px`;
  animEl.textContent = '💥';
  animEl.classList.remove('hidden');
  animEl.style.animation = 'none';
  void animEl.offsetWidth; // reflow pour relancer l'animation
  animEl.style.animation = 'replayKillPop .7s ease forwards';
  setTimeout(() => animEl.classList.add('hidden'), 800);
}

// Cache des images webp pour le replay
const _replayImgCache = {};
function getReplayImg(src) {
  if (_replayImgCache[src]) return _replayImgCache[src];
  const img = new Image();
  img.src = src;
  img.onload = () => {
    // Redessiner si le replay est ouvert
    const canvas = document.getElementById('replay-canvas');
    if (canvas && _moveLog[_replayIdx]) drawReplayBoard(_moveLog[_replayIdx].snapshot);
  };
  _replayImgCache[src] = img;
  return img;
}

function drawReplayBoard(snapshot) {
  const canvas = document.getElementById('replay-canvas');
  if (!canvas) return;
  ...
  // ── Damier (couleurs exactes du vrai jeu) ──
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      if (r === 4 && c === 4) continue; // Nid dessiné séparément
      ctx.fillStyle = (r + c) % 2 === 0 ? '#32305A' : '#06040F';
      ctx.fillRect(c * cs, r * cs, cs, cs);
    }
  }

  // ── Nid Sacré (case 4,4) ──
  const nx = 4 * cs, ny = 4 * cs;
  const grdNid = ctx.createRadialGradient(nx + cs/2, ny + cs/2, 0, nx + cs/2, ny + cs/2, cs * 0.8);
  grdNid.addColorStop(0, 'rgba(212,160,23,.7)');
  grdNid.addColorStop(1, 'rgba(120,70,0,.15)');
  ctx.fillStyle = grdNid;
  ctx.fillRect(nx, ny, cs, cs);
  // Croix du Nid
  ctx.strokeStyle = 'rgba(212,160,23,.85)';
  ctx.lineWidth = cs * 0.055;
  ctx.beginPath();
  ctx.moveTo(nx + cs/2, ny + cs * .18); ctx.lineTo(nx + cs/2, ny + cs * .82);
  ctx.moveTo(nx + cs * .18, ny + cs/2); ctx.lineTo(nx + cs * .82, ny + cs/2);
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(nx + cs/2, ny + cs/2, cs * 0.28, 0, Math.PI * 2);
  ctx.strokeStyle = 'rgba(212,160,23,.7)'; ctx.lineWidth = cs * 0.045; ctx.stroke();
  // Bordure inset
  ctx.strokeStyle = 'rgba(212,160,23,.9)';
  ctx.lineWidth = cs * 0.05;
  ctx.strokeRect(nx + cs * 0.05, ny + cs * 0.05, cs * 0.9, cs * 0.9);
  ...
  for (const p of snapshot.pieces) {
    ...
    if (p.dead) {
      // Dépouille : cercle sombre + contour blanc faible + image grisée
      ctx.globalAlpha = 0.6;
      ctx.beginPath(); ctx.arc(x, y, rad, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(20,18,40,.85)'; ctx.fill();
      ctx.strokeStyle = 'rgba(255,255,255,.35)'; ctx.lineWidth = cs * 0.055; ctx.stroke();
      const imgSrc = SYM[p.type];
      if (imgSrc) {
        const img = getReplayImg(imgSrc);
        if (img.complete && img.naturalWidth > 0) {
          ctx.save();
          ctx.beginPath(); ctx.arc(x, y, rad - cs * 0.03, 0, Math.PI * 2); ctx.clip();
          ctx.filter = 'grayscale(100%) brightness(0.5)';
          const iSize = rad * 1.85;
          ctx.drawImage(img, x - iSize / 2, y - iSize / 2, iSize, iSize);
          ctx.filter = 'none';
          ctx.restore();
        }
      }
      ...
      // Pièce vivante : dégradé radial comme drawPiece()
      ctx.globalAlpha = 1;
      const fc = CFILL[p.color] || '#888';
      const grd = ctx.createRadialGradient(x - rad * .2, y - rad * .25, rad * .05, x, y, rad);
      grd.addColorStop(0, lighten(fc, .38));
      grd.addColorStop(.55, fc);
      grd.addColorStop(1, darken(fc, .32));
      ctx.beginPath(); ctx.arc(x, y, rad, 0, Math.PI * 2);
      ctx.fillStyle = grd; ctx.fill();
      ctx.strokeStyle = lighten(fc, .55); ctx.lineWidth = cs * 0.04; ctx.stroke();
      // Image
      const imgSrc = SYM[p.type];
      if (imgSrc) {
        const img = getReplayImg(imgSrc);
        if (img.complete && img.naturalWidth > 0) {
          ctx.save();
          ctx.beginPath(); ctx.arc(x, y, rad - cs * 0.02, 0, Math.PI * 2); ctx.clip();
          const iSize = rad * 1.85;
          ctx.drawImage(img, x - iSize / 2, y - iSize / 2, iSize, iSize);
          ctx.restore();
        }
  ...
  // ── Numéro de tour (coin bas-droite) ──
  ctx.globalAlpha = 0.55;
  ctx.fillStyle = '#E0DCFF';
  ctx.font = `bold ${Math.round(cs * 0.38)}px "Exo 2", sans-serif`;
  ctx.textAlign = 'right';
  ctx.textBaseline = 'bottom';
  ctx.fillText(`Tour ${snapshot.turn}`, size - cs * 0.12, size - cs * 0.08);
  ctx.globalAlpha = 1;
}

function replayPrev() { if (_replayIdx < _moveLog.length - 1) { _replayIdx++; renderReplayFrame(); } }
function replayNext() { if (_replayIdx > 0) { _replayIdx--; renderReplayFrame(); } }
...
function isQueenTrapped(color) {
  ...
  // Si la colonie a un Scarabée vivant → pas encerclée (il peut déplacer les dépouilles)
  const hasNecro = pl.pieces.some(p=>!p.dead&&p.type==='necromobile'&&p.color===color);
  if (hasNecro) return false;
  // Flood fill depuis la Reine.
  // MURS : dépouilles (même ghost-SP) + bords du plateau.
  // TRAVERSABLE : cases vides + pièces vivantes.
  // La zone est CLOSE si le flood fill ne touche jamais aucune case hors-plateau.
  // Autrement dit : si le fill n'a jamais tenté de franchir un bord → encerclée.
  // On détecte ça en regardant si, pour chaque case visitée,
  // TOUS ses 8 voisins sont soit visités, soit des murs (dépouilles).
  // Si un voisin est hors-plateau → il y a un bord accessible = pas close.
  // Mais bord = mur dans notre logique, donc le fill ne sort jamais.
  // La vraie question : est-ce que la zone peut s'étendre à l'infini (= pas close) ?
  // Réponse : NON par définition si on ne franchit jamais un bord.
  // DONC : le fill est toujours fini. L'encerclement est vérifié si
  // le fill NE CONTIENT AUCUNE CASE en bordure du plateau (r=0/8 ou c=0/8)
  // qui soit vide ou vivante (= non-mur).
  ...
  // Encerclée = la zone accessible ne contient aucune case de bord du plateau
  // (car si elle touche un bord via une case non-dépouille, elle peut y aller)
  for (const key of visited) {
    const [r,c] = key.split(',').map(Number);
    if (r===0||r===8||c===0||c===8) return false; // bord accessible = pas encerclée
  }
  return true; // zone intérieure totalement close
}
...
function checkGhostResurrections() {
  ...
  for (const entry of toRevive) {
    ...
    showInfoText('👻 Résurrection !',
      ...
      CCSS[p.color]);
    logMove({ color: CCSS[p.color], text: `<b style="color:${CCSS[p.color]}">${pieceName} ${colName}</b> ressuscite ! (fantôme)` });
  }
  renderBoard();
}
...
function eliminateTrapped(color, killerColor) {
  ...
  if (kpl&&kpl.alive) {
    for (const p of pl.pieces) {
      if (!p.dead && p.type !== 'chef') { // la Reine ne change PAS de couleur
        p.color = killerColor; kpl.pieces.push(p);
      }
    }
    // La Reine reste morte (dépouille de sa couleur d'origine)
    pl.pieces = pl.pieces.filter(p => p.dead || p.type === 'chef');
    // Marquer la Reine comme dépouille
...
function finishTurn(){
  ...
  // Avancer le tour AVANT tout le reste (y compris SP) pour que _resumeAfterSP soit correct
...
function execAIMove(mv,color,onDone){
  ...
  if(type==='move'){
    ...
    const acts = getActionsWithSP(piece);
    if (acts.resurrectMode) {
      // Coccinelle SP : résurrection dans la colonie
      ...
      // Scarabée normal ou SP free-move-corpse : déplacer la dépouille
      removeFromBoard(target);
      animMove(piece,toR,toC).then(()=>{
...
function getActionsWithSP(piece) {
  ...
  switch (sp.type) {
    ...
    case 'free-move-corpse': {
      ...
      return { moves: base.moves, kills: base.kills, diplT: base.diplT, necroT: corpseTargets, freeMoveCorpse: true };
    }
    case 'area-kill': {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (document.visibilityState === 'visible') {
      ...
      if (isGameScreen && G && !G.over) {
        // Vérifier si les pièces DOM sont présentes (pas seulement les cellules vides)
        const pieceDivs = boardEl ? boardEl.querySelectorAll('.piece').length : 0;
        const expectedPieces = Object.values(G.players||{}).reduce((n,pl)=>
          n + pl.pieces.filter(p=>!p.dead).length, 0);
        const domStale = !boardEl || boardEl.children.length === 0 || pieceDivs < expectedPieces;
        if (domStale) {
