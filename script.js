...
// ── Partage : mobile natif ou popup desktop ──
async function doShare() {
  const url   = window.location.href.split('?')[0];
  const text  = '🪲 Je joue à IN-SECT, un jeu de stratégie avec des insectes ! Reine, Araignée, Mouche... 4 colonies en guerre. Essaie !';
  const title = "IN-SECT — L'Échiquier des Colonies";

  // Mobile : Web Share API disponible → feuille de partage native
  if (navigator.share) {
    try {
      await navigator.share({ title, text, url });
      onShareSuccess();
    } catch (err) {
      if (err.name !== 'AbortError') toast('Partage annulé.');
    }
    return;
  }

  // Desktop : afficher notre popup de partage
  showDesktopShareModal(url, text);
}

// Ouvre le popup de partage desktop
function showDesktopShareModal(url, text) {
  // Supprimer un éventuel popup précédent
  document.getElementById('share-desktop-modal')?.remove();

  const encoded = encodeURIComponent(url);
  const encodedText = encodeURIComponent(text + ' ' + url);

  const modal = document.createElement('div');
  modal.id = 'share-desktop-modal';
  modal.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:600;display:flex;align-items:flex-end;justify-content:center;padding-bottom:var(--safe-bot);backdrop-filter:blur(6px);';

  modal.innerHTML = `
    <div style="background:linear-gradient(170deg,#0D0B1E,#08060F);border:1px solid #8A6800;border-radius:18px 18px 0 0;width:100%;max-width:500px;padding:20px 20px 28px;display:flex;flex-direction:column;gap:12px;">
      <div style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #8A6800;padding-bottom:12px;">
        <span style="font-family:'Exo 2',sans-serif;font-size:.8rem;font-weight:900;letter-spacing:.2em;color:#D4A017;">🤝 PARTAGER LE JEU</span>
        <button onclick="document.getElementById('share-desktop-modal').remove()" style="background:transparent;border:1px solid #8A6800;border-radius:7px;color:#8A6800;font-size:.9rem;width:32px;height:32px;cursor:pointer;">✕</button>
      </div>
      <div style="display:flex;flex-direction:column;gap:8px;">
        <a href="mailto:?subject=${encodeURIComponent(title)}&body=${encodedText}" target="_blank" style="display:flex;align-items:center;gap:12px;padding:13px 16px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:10px;text-decoration:none;color:#E0DCFF;font-family:'Exo 2',sans-serif;font-size:.88rem;font-weight:700;cursor:pointer;" onclick="onShareClick()">
          <span style="font-size:1.4rem;">✉️</span> Par e-mail
        </a>
        <a href="https://wa.me/?text=${encodedText}" target="_blank" style="display:flex;align-items:center;gap:12px;padding:13px 16px;background:rgba(37,211,102,.08);border:1px solid rgba(37,211,102,.25);border-radius:10px;text-decoration:none;color:#E0DCFF;font-family:'Exo 2',sans-serif;font-size:.88rem;font-weight:700;cursor:pointer;" onclick="onShareClick()">
          <span style="font-size:1.4rem;">💬</span> WhatsApp
        </a>
        <a href="https://t.me/share/url?url=${encoded}&text=${encodeURIComponent(text)}" target="_blank" style="display:flex;align-items:center;gap:12px;padding:13px 16px;background:rgba(0,136,204,.08);border:1px solid rgba(0,136,204,.25);border-radius:10px;text-decoration:none;color:#E0DCFF;font-family:'Exo 2',sans-serif;font-size:.88rem;font-weight:700;cursor:pointer;" onclick="onShareClick()">
          <span style="font-size:1.4rem;">✈️</span> Telegram
        </a>
        <div style="display:flex;gap:8px;">
          <div id="share-url-box" style="flex:1;padding:10px 12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:10px;font-family:monospace;font-size:.72rem;color:#9090C8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${url}</div>
          <button onclick="copyShareUrl('${url}')" style="padding:10px 14px;background:rgba(128,80,255,.2);border:1px solid rgba(128,80,255,.4);border-radius:10px;color:#C090FF;font-family:'Exo 2',sans-serif;font-size:.78rem;font-weight:700;cursor:pointer;white-space:nowrap;">📋 Copier</button>
        </div>
      </div>
    </div>`;
...
// Appelé après chaque partage réussi (bouton cliqué dans le popup ou share API)
...
const PDESC = {
  chef:       "Se déplace librement en ligne droite (ortho + diag), à n'importe quelle distance. <b>Tue</b> en se posant sur l'ennemi — vous placez la dépouille 💀. Seule pièce sur le <b>Nid Sacré 👑</b>. Sa mort = fin de colonie.",
  assassin:   "<b>Tue</b> une pièce ennemie puis <b>retourne à sa case de départ</b>. La dépouille reste sur la case d'attaque.",
  reporter:   "Se pose sur une case <b>vide</b>, puis <b>élimine toutes les pièces ennemies adjacentes</b> (ortho + diag). Peut tuer plusieurs cibles en un coup.",
  necromobile:"<b>Déplace une dépouille 💀</b> vers n'importe quelle case vide. <b>Pièce vitale</b> : sans elle, la Reine peut être encerclée.",
  diplomate:  "<b>Transporte une pièce ennemie vivante</b> vers n'importe quelle case vide — sans la tuer.",
  militant:   "<b>Portée max 2 cases.</b> Tue une pièce ennemie en se posant dessus. 🚫 Ne peut pas attaquer la Reine sur le Nid Sacré."
};
...
function goMenu() {
  ...
  // Réinitialiser le showcase pour éviter qu'il bloque les clics sur les modals
  const showcase = document.getElementById('piece-showcase');
...
function updatePieceInfo(piece) {
  const displayImg = piece.type === 'chef' ? 'reine_guepe_512.png' : SYM[piece.type];

  // Showcase : image + nom complet + description
  const showcase     = document.getElementById('piece-showcase');
  const showcaseImg  = document.getElementById('piece-showcase-img');
  const showcaseName = document.getElementById('piece-showcase-name');
  const showcaseDesc = document.getElementById('piece-showcase-desc');
  if (showcaseImg) { showcaseImg.src = displayImg; showcaseImg.alt = PNAME[piece.type]; }
  if (showcaseName) { showcaseName.textContent = PFULL[piece.type] || PNAME[piece.type]; showcaseName.style.color = CCSS[piece.color]; }
  if (showcaseDesc) { showcaseDesc.innerHTML = PDESC[piece.type]; }
  if (showcase) showcase.classList.add('active');

  // Barre du bas : juste le nom de la pièce (couleur colonie) + hint si aucune pièce sélectionnée
  const piName = document.getElementById('pi-name');
  const piHint = document.getElementById('pi-hint');
  const pi = document.getElementById('pi');
  if (piName) { piName.textContent = PNAME[piece.type]; piName.style.color = CCSS[piece.color]; }
  if (piHint) piHint.style.display = 'none';
  if (pi) { pi.style.borderColor = CCSS[piece.color]+'44'; pi.style.boxShadow = `0 0 10px ${CGLOW[piece.color]}`; }
}
...
function humanClickCell(r,c){
  ...
  if (r===LAB.r&&c===LAB.c&&!clicked&&!G.sel){
    ...
    const showcaseName = document.getElementById('piece-showcase-name');
    const showcaseDesc = document.getElementById('piece-showcase-desc');
    if (showcaseImg) { showcaseImg.src = 'reine_guepe_512.png'; showcaseImg.alt = 'Nid Sacré'; }
    if (showcaseName) { showcaseName.textContent = '👑 Nid Sacré'; showcaseName.style.color = '#D4A017'; }
    if (showcaseDesc) { showcaseDesc.innerHTML = "Reine ici = <b>2 actions/tour</b> (duel) ou <b>rejeu après chaque adversaire</b> (guerre totale). <b>Immunisée contre les Fourmis.</b> Les autres pièces la traversent sans s'arrêter. Elle peut quitter le Nid librement."; }
    if (showcase) showcase.classList.add('active');
    return;
  }
...
// Niveau 1 — aléatoire, mais saisit toujours une opportunité de tuer une Reine
function aiLevel1(color){
  const all=getAllMoves(color);
  if(!all.length) return null;
  // Si une Reine ennemie est à portée, l'IA la tue (même en mode basique)
