...
// Fourmis vertes et rouges regardent vers la gauche
function getSYM(type, color) {
  if (type === 'militant' && (color === 'green' || color === 'red')) return 'fourmi_gauche_512.png';
  return SYM[type];
}
...
function _updateSoundTracks() {
  const tm = document.getElementById('sound-track-music');
  const ts = document.getElementById('sound-track-sfx');
  if (tm) tm.classList.toggle('on', !_muted);
  if (ts) ts.classList.toggle('on', !_sfxMuted);
  // Boutons icône
  ['snd-btn','snd-btn-game'].forEach(id => {
    const b = document.getElementById(id);
    if (b) b.textContent = (_muted && _sfxMuted) ? '🔇' : (!_sfxMuted && !_muted) ? '🔊' : '🎵';
  });
}
...
function toggleMute() { toggleMusic(); } // compatibilité
function openSoundModal() { showModal('sound'); _updateSoundTracks(); }
...
function buildBoard() {
  ...
  for (const c of G.order) {
    for (const p of G.players[c].pieces) {
      ...
      pe.appendChild(cv);
      drawPiece(cv, p.color, getSYM(p.type, p.color), p.dead, false);
...
function renderBoard() {
  ...
  for (const p of allPieces) {
    ...
    if (cv) drawPiece(cv, p.color, getSYM(p.type, p.color), p.dead, isSel);
...
function updatePieceInfo(piece) {
  // Showcase AU-DESSUS du plateau : grande image + nom coloré + description
  const displayImg   = piece.type === 'chef' ? 'guepe.png' : getSYM(piece.type, piece.color);
  ...
  // Barre basse .pi : juste nom + description, sans image
  const pi = document.getElementById('pi');
  pi.innerHTML = `<div class="pi-info"><div class="pin" style="color:${CCSS[piece.color]}">${PNAME[piece.type]}</div><div class="pid">${PDESC[piece.type]}</div></div>`;
...
function executeKill(killer, victim) {
  ...
  if (pe) {
    ...
    setTimeout(() => {
      ...
      const cv = pe.querySelector('canvas');
      if (cv) drawPiece(cv, victim.color, getSYM(victim.type, victim.color), true, false);
...
function execReporterNuee(reporter,isOrtho){
  ...
  for(const[dr,dc]of dirs){
    ...
    if(t&&!t.dead&&t.color!==reporter.color){
      ...
      if(pe){pe.classList.add('flash-kill');setTimeout(()=>{pe.classList.remove('flash-kill');const cv=pe.querySelector('canvas');if(cv)drawPiece(cv,t.color,getSYM(t.type,t.color),true,false);pe.style.filter='none';pe.style.zIndex='2';},460);}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // La barre de chargement s'anime en CSS (2s), le bouton ENTRER attend le clic
  // splashEnter() est appelé par le bouton dans index.html

});

function splashEnter() {
  // Débloque l'audio (obligatoire après un geste utilisateur sur mobile)
