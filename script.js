...
function isQueenTrapped(color) {
  ...
  // Stratégie : flood fill depuis TOUTES les cases de bord non-dépouille.
  // Si la case de la Reine est atteignable → elle n'est PAS encerclée.
  // Si elle est NON atteignable → elle est encerclée par les dépouilles + bords.
  const visited = new Set();
  const queue = [];

  // Amorcer depuis toutes les cases de bord qui ne sont pas des dépouilles
  for (let r = 0; r < 9; r++) {
    for (let c = 0; c < 9; c++) {
      if (r !== 0 && r !== 8 && c !== 0 && c !== 8) continue; // seulement les bords
      const cell = G.board[r][c];
      if (cell && cell.dead) continue; // dépouille = mur même sur le bord
      const key = `${r},${c}`;
      if (!visited.has(key)) { visited.add(key); queue.push([r,c]); }
    }
  }

  // Flood fill vers l'intérieur — dépouilles bloquent
  while (queue.length) {
    const [r,c] = queue.shift();
    // Si on atteint la case de la Reine → pas encerclée
    if (r === chef.r && c === chef.c) return false;
    for (const [dr,dc] of DIRS8) {
  ...
  return true; // Reine non atteinte depuis l'extérieur = encerclée
}
