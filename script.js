...
function isQueenTrapped(color) {
  ...
  // Flood-fill depuis l'EXTÉRIEUR (tous les bords) vers l'intérieur.
  // Murs : uniquement les dépouilles (cell.dead). Pièces vivantes = traversables.
  // Si la Reine est atteignable depuis un bord → pas encerclée.
  // Si le fill envahit tout le plateau sans atteindre la Reine → encerclée.
  ...
  for (let i = 0; i < 9; i++) {
    for (const [r,c] of [[0,i],[8,i],[i,0],[i,8]]) {
      ...
      if (cell && cell.dead) continue; // dépouille = mur, ne pas entrer
      visited.add(key);
      queue.push([r,c]);
    }
  }
  while (queue.length) {
    ...
    if (r === chef.r && c === chef.c) return false; // Reine atteignable → pas piégée
    for (const [dr,dc] of DIRS8) {
  ...
  return true; // Reine jamais atteinte depuis l'extérieur → encerclée
}
