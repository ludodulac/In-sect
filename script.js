...
function updateStatusBar() {
  ...
  const resetBtn = document.getElementById('menu-score-reset');
  if (scoreEl) {
    const w = parseInt(lsGet('insect_wins','0'),10);
    const l = parseInt(lsGet('insect_losses','0'),10);
    if (w > 0 || l > 0) {
      scoreEl.innerHTML = `<span class="score-w">▲ ${w} Victoire${w!==1?'s':''}</span> · <span class="score-l">▼ ${l} Défaite${l!==1?'s':''}</span>`;
...
function logMove(entry) {
  ...
  if (_moveLog.length > 3) _moveLog.length = 3;
  renderMoveLog();
}

function renderMoveLog() {
  const el = document.getElementById('move-log-entries');
  if (!el) return;
  if (!_moveLog.length) { el.innerHTML = '<div class="move-log-entry"><span class="mle-txt" style="color:var(--text-muted);font-style:italic;">Aucun coup joué</span></div>'; return; }
...
function resumeGame() {
  ...
  updateTurnUI();
  updateToggleUI();
  // Reprendre le tour IA si c'est à une IA de jouer
  if (!G.players[cur()].human) {
    _aiTimer = setTimeout(() => runAILoop(), 800);
  }
}
...
function scoreAIMove(mv,color){
  ...
  if(mv.type==='necro'){
    // Valeur selon le type de pièce ressuscitée
    const PIECE_VAL = { chef:0, necromobile:350, reporter:300, assassin:280, araignee:260, coccinelle:240, fourmi:200 };
    const val = PIECE_VAL[mv.target.type] || 200;
    s += val;
    // Bonus si la pièce ressuscitée est forte et l'IA en manque
    const myPieces = G.players[color] ? G.players[color].pieces.filter(p=>!p.dead&&p.color===color) : [];
    const myCount = myPieces.length;
    if (myCount <= 3) s += 150; // urgence si peu de pièces
  }
  if(mv.type==='dipl'&&mv.target.type==='chef')s+=80;
  s+=Math.random()*8; // bruit réduit en expert
  return s;
}
...
function evalState(state, color) {
  ...
  for (const c of G.order) {
    ...
    // Gros malus si la reine de notre couleur est exposée
    if (c === color && pl.alive) {
      const chef = pl.pieces.find(p => p.type==='chef' && !p.dead);
      if (chef) {
        // Compter les menaces sur la reine dans l'état cloné
        let threats = 0;
...
function minimaxScore(mv, myColor) {
  ...
  for (const opp of opponents) {
    ...
    for (const omv of oppMoves.slice(0, 20)) { // limiter pour perf
...
function aiLevel3(color){
  ...
  // Priorité absolue : tuer la reine ennemie maintenant
  ...
  if (queenThreatened) {
    ...
    if (chef) {
      ...
      if (escapes.length) {
        // Choisir la fuite avec le meilleur minimax
  ...
  // Filtre : la Reine ne va jamais sur une case menacée sauf dernier recours
  const safe = all.filter(mv =>
    !(mv.piece.type==='chef' && mv.type==='move' && countThreatsTo(mv.toR, mv.toC, color) > 0)
  );
