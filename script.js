...
function isQueenTrapped(color) {
  ...
  // Flood fill depuis la Reine. Murs = dépouilles. !inB = ignoré (mur).
  // Encerclée si le fill n'atteint jamais une case de bord du plateau libre (non-dépouille).
  // Une case de bord libre = une sortie possible vers l'extérieur.
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) continue; // hors plateau = mur, on ignore
  ...
  // Vérifier si une case de bord libre est dans la zone accessible
  for (const key of visited) {
    const [r,c] = key.split(',').map(Number);
    if (r===0 || r===8 || c===0 || c===8) {
      const cell = G.board[r][c];
      if (!cell || !cell.dead) return false; // bord libre accessible = pas encerclée
    }
  }

  return true; // aucun bord libre accessible = encerclée
}
