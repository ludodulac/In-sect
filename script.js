...
function startGame() {
  ...
  // Afficher les toggles dans les deux modes
  // En mode 1 IA : seulement le toggle super pouvoirs (pas le pacte)
  const togs = document.getElementById('game-toggles');
  if (togs) togs.style.display = 'flex';
  const gtogPact = document.getElementById('gtog-pact');
  const pactPanel = document.getElementById('pact-panel');
  if (gtogPact) gtogPact.style.display = _mode === 1 ? 'none' : '';
  if (pactPanel) pactPanel.style.display = _mode === 1 ? 'none' : '';
  updateToggleUI();
...
function doKill(piece,victim,toR,toC){
  ...
  if (isPactBlocked(piece.color, victim.color)) { toast('🤝 Le pacte empêche cette attaque !'); G.sel=null; renderBoard(); return; }
...
function finishTurn(){
  ...
  // Vérifier fin de pacte
  checkPactEnd();
  // Déclencher super pouvoirs
  checkSPTrigger();
  // Proposer pacte si conditions remplies (en mode 4 joueurs)
  if (!G.mode1) tryTriggerPact();
...
function aiLevel2(color){
  ...
  // Filtre : la Reine ne se déplace jamais vers une case menacée, sauf dernier recours
...
function togglePactOption() {
  _optPact = !_optPact;
  updateToggleUI();
  toast(_optPact ? '🤝 Pacte de non-agression activé' : '🤝 Pacte désactivé');
}
...
function updateToggleUI() {
  const cp = document.getElementById('gtog-check-pact');
  const cs = document.getElementById('gtog-check-sp');
...
function tryTriggerPact() {
  if (!_optPact || G.mode1 || G.pact || G.pactProposed) return;
  if (_gameCaps < 10) return;
  const alive = G.order.filter(c => G.players[c].alive);
  if (alive.length < 3) return;  // pas assez de joueurs pour un pacte
  G.pactProposed = true;

  // Tirage au sort de 2 colonies parmi les vivants
  const shuffled = [...alive].sort(() => Math.random() - .5);
  const [c1, c2] = shuffled;

  const col1 = CCSS[c1], col2 = CCSS[c2];
  const n1 = CNAME_SHORT[c1], n2 = CNAME_SHORT[c2];

  const txt = document.getElementById('pact-proposal-text');
  if (txt) txt.innerHTML =
    `<b>Tirage au sort :</b><br>` +
    `<span style="color:${col1};font-weight:900;">● ${n1}</span> &amp; ` +
    `<span style="color:${col2};font-weight:900;">● ${n2}</span><br><br>` +
    `Ces deux colonies ne peuvent plus se causer aucun dégât.`;

  G._pendingPact = [c1, c2];
  showModal('pact');
}

function acceptPact() {
  if (!G._pendingPact) return;
  const [c1, c2] = G._pendingPact;
  G.pact = { colors: [c1, c2] };
  G._pendingPact = null;
  hideModal('pact');
  updatePactBanner();
  toast(`🤝 Pacte entre ${CNAME_SHORT[c1]} & ${CNAME_SHORT[c2]} conclu !`);
}
...
function updatePactBanner() {
  const banner = document.getElementById('pact-banner');
  if (!banner) return;
  if (!G.pact) { banner.style.display = 'none'; return; }
  const [c1, c2] = G.pact.colors;
  const col1 = CCSS[c1], col2 = CCSS[c2];
  banner.style.display = '';
  banner.style.background = `linear-gradient(90deg, ${col1}22, ${col2}22)`;
  banner.style.border = `1px solid ${col1}55`;
  banner.innerHTML =
    `🤝 <span style="color:${col1}">● ${CNAME_SHORT[c1]}</span> & ` +
    `<span style="color:${col2}">● ${CNAME_SHORT[c2]}</span> — Pacte de non-agression actif`;
}
...
// Vérifie si le pacte doit prendre fin (une colonie hors pacte éliminée)
function checkPactEnd() {
  if (!G.pact) return;
  const [c1, c2] = G.pact.colors;
  // Le pacte prend fin si une colonie extérieure est éliminée
  const outsiders = G.order.filter(c => c !== c1 && c !== c2);
  // Si plus d'outsider vivant, le pacte n'a plus de sens → on le dissout
  const aliveOutsiders = outsiders.filter(c => G.players[c] && G.players[c].alive);
  if (aliveOutsiders.length === 0) {
    G.pact = null;
    updatePactBanner();
    toast('🤝 Le pacte est terminé — le dernier outsider a été éliminé');
  }
}
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const SP_TYPES = {
  militant:    'queen-move',    // Fourmi → se déplace comme une Reine
  reporter:    'area-kill',     // Mouche → tue dans les 8 directions
  diplomate:   'move-allies',   // Coccinelle → peut déplacer ses propres pièces
  assassin:    'double-kill',   // Araignée → double kill dans la ligne
  chef:        'invincible',    // Reine Guêpe → invincible 4 coups
  necromobile: 'corpse-warp',   // Scarabée → se téléporte sur une dépouille
};
const SP_DESC = {
...
function checkSPTrigger() {
  if (!_optSP) return;
  if (_gameTurns < G.spNextTrigger) return;
  G.spNextTrigger += 10;
  launchSPAnimation();
}

function launchSPAnimation() {
  // Tirer la case cible au sort (toutes les 81 cases ont la même chance)
  const targetR = Math.floor(Math.random() * 9);
  const targetC = Math.floor(Math.random() * 9);

  // Construire l'ordre spirale des cases depuis le centre (4,4)
  const spiralOrder = buildSpiralOrder();

  // Trouver l'index de la cible dans la spirale
  const targetIdx = spiralOrder.findIndex(([r,c]) => r === targetR && c === targetC);

  // Animation sur le canvas FX
  const boardEl = document.getElementById('board');
  if (!boardEl) { onSPLand(targetR, targetC); return; }
  const bRect = boardEl.getBoundingClientRect();
  const cs = getCellSize();

  let currentIdx = 0;
  const totalCycles = 3; // nombre de tours complets
  const totalFrames = spiralOrder.length * totalCycles + targetIdx + spiralOrder.length;
  ...
  function step() {
    if (G.over) return;
    // Effacer ancien highlight
    if (lastHighlight) {
      const cell = document.getElementById(`cell-${lastHighlight[0]}-${lastHighlight[1]}`);
      if (cell) cell.classList.remove('sp-aura-spin');
    }

    const progress = frame / totalFrames;
    // Ralentissement : vitesse élevée au début, lente à la fin
    const speedFactor = 1 - Math.pow(progress, 0.5) * 0.85;
    const delay = Math.max(18, 120 * (1 - speedFactor) + 18);

    let pos;
    if (frame < spiralOrder.length * totalCycles) {
      pos = frame % spiralOrder.length;
    } else {
      // Dernière passe : on s'arrête au targetIdx
      const extra = frame - spiralOrder.length * totalCycles;
      if (extra >= targetIdx) {
        // Arrivé !
        const cell = document.getElementById(`cell-${targetR}-${targetC}`);
        if (cell) cell.classList.add('sp-aura-spin');
        setTimeout(() => {
          if (cell) cell.classList.remove('sp-aura-spin');
          onSPLand(targetR, targetC);
        }, 800);
        return;
      }
      pos = extra % spiralOrder.length;
    }

    const [r, c] = spiralOrder[pos];
  ...
  // Ajouter style spin temporaire
  const style = document.createElement('style');
  style.id = 'sp-spin-style';
  style.textContent = `.sp-aura-spin{background:rgba(200,120,255,.35)!important;box-shadow:inset 0 0 10px rgba(220,140,255,.8),0 0 14px rgba(180,80,255,.7)!important;transition:background .05s,box-shadow .05s;}`;
  if (!document.getElementById('sp-spin-style')) document.head.appendChild(style);

  toast('✨ Super pouvoir en route…');
  step();
}
...
function onSPLand(r, c) {
  ...
  if (piece && !piece.dead) {
    ...
    G.spCells[`${r},${c}`] = true;
    renderBoard();
    toast(`✨ Le super pouvoir attend sur la case (${r+1},${c+1}) — première pièce qui s'y pose le reçoit !`);
  }
}
...
function showSPResult(piece, spType) {
  ...
  if (txt) txt.innerHTML =
    `<b style="color:${col}">${name}</b> de la <b style="color:${col}">${CNAME_SHORT[piece.color]}</b> a reçu un super pouvoir !<br><br>` +
    `<b>✨ ${SP_DESC[spType]}</b>`;
  showModal('sp-result');
}

function closeSPResult() { hideModal('sp-result'); }
