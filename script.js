...
function aiLevel1(color){
  ...
  // 1. Tuer la Reine ennemie si c'est possible — toujours
  ...
  // 4. Parmi les coups sûrs : légère préférence pour les kills (30% de chance de les prioriser)
  if(Math.random()<0.30){
...
function aiLevel3(color){
  ...
  // Priorité 1 : sauver sa propre reine si menacée (avant tout)
  const queenThreatened = isQueenThreatened(color);
  if (queenThreatened) {
    const chef = G.players[color].pieces.find(p=>p.type==='chef'&&!p.dead);
    if (chef) {
  ...
  // Priorité 2 : tuer la reine ennemie — seulement si ma reine n'est pas en danger
  // ou si le kill ne m'expose pas (case destination sûre)
  if (!queenThreatened) {
    const queenKill = all.find(mv =>
      mv.type==='kill' && mv.target.type==='chef' &&
      countThreatsTo(mv.toR, mv.toC, color) === 0
    );
    if (queenKill) return queenKill;
  }
