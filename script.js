...
function startGame() {
  ...
  G = {
    ...
    spNextTrigger: 15,    // prochain déclenchement (tous les 15 coups)
  };
...
function toggleSPOption() {
  ...
  updateToggleUI();
  if (_optSP) {
    // Réactivation : prochain SP dans 5 coups
    G.spNextTrigger = _gameTurns + 5;
    toast('✨ Super pouvoirs activés — prochain dans 5 coups');
  } else {
    toast('✨ Super pouvoirs désactivés');
  }
}
...
function executeDoubleKill(piece, victim, fromR, fromC) {
  ...
  if (second) {
    ...
    // Placement intelligent de la 2e dépouille :
    // 1. Case derrière la 1ère dépouille sur la même ligne/diag
    // 2. Plus loin sur la même ligne/diag
    // 3. Autre case libre de la même ligne/diag
    // 4. Case vide la plus proche
    // Nid Sacré interdit aux dépouilles
    function isValidCorpseCell(r, c) {
      return inB(r,c) && !G.board[r][c] && !(r===LAB.r && c===LAB.c);
    }
    let placed = false;
    // 1. Juste derrière la 1ère dépouille (fromR+dr, fromC+dc)
    const d2r = fromR + dr, d2c = fromC + dc;
    if (isValidCorpseCell(d2r, d2c)) {
      placeOnBoard(second, d2r, d2c); placed = true;
    }
    // 2 & 3. Plus loin sur la même ligne/diag
    if (!placed) {
      let sr = fromR + dr*2, sc = fromC + dc*2;
      while (inB(sr, sc)) {
        if (isValidCorpseCell(sr, sc)) { placeOnBoard(second, sr, sc); placed = true; break; }
        sr += dr; sc += dc;
      }
    }
    // 4. Case vide la plus proche (BFS)
    if (!placed) {
      ...
      outer: while (queue.length) {
        ...
        for (const [ddr,ddc] of DIRS8) {
          const rr = cr+ddr, rc = cc+ddc;
          const key = `${rr},${rc}`;
          if (!inB(rr,rc) || seen.has(key)) continue;
          seen.add(key);
          if (isValidCorpseCell(rr, rc)) { placeOnBoard(second, rr, rc); placed = true; break outer; }
          queue.push([rr, rc]);
        }
      }
    }
