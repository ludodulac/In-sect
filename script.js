...
function updateStatusBar() {
  ...
  // Score victoires/défaites — toujours visible
...
function isQueenTrapped(color) {
  ...
  // Condition : le Scarabée allié doit être mort pour que l'encerclement soit valide
  ...
  // - MURS : dépouilles (même avec SP ghost) + bords du plateau
  // - TRAVERSABLE : cases vides + pièces vivantes (la Reine peut potentiellement s'y déplacer)
  // Encerclée = flood fill ne trouve AUCUNE case vide accessible
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) continue;           // bord = mur
      const key = `${nr},${nc}`; if (visited.has(key)) continue;
      const cell = G.board[nr][nc];
      if (cell && cell.dead) continue;      // dépouille (quelle que soit son SP) = mur
      visited.add(key); queue.push([nr,nc]); // case vide ou pièce vivante = traversable
    }
  }
  // Chercher une case vide dans la zone accessible
  for (const key of visited) {
    const [r,c] = key.split(',').map(Number);
    if (r===chef.r && c===chef.c) continue;
    if (!G.board[r][c]) return false; // case vide trouvée = pas encerclée
  }
  return true; // aucune case vide accessible = encerclée
}
...
function checkGhostResurrections() {
  ...
  for (const entry of toRevive) {
    ...
    // Supprimer le marqueur ghost — la pièce est vivante, plus de SP résiduel
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    // Retour sur la page : recharger si la partie a disparu du DOM
    if (document.visibilityState === 'visible') {
      const boardEl = document.getElementById('board');
      const isGameScreen = document.getElementById('s-game') && !document.getElementById('s-game').classList.contains('hidden');
      if (isGameScreen && boardEl && boardEl.children.length === 0 && G && !G.over) {
        // Le DOM a été vidé par iOS — reconstruire
  ...
  // Sauvegarde automatique toutes les 15 secondes pendant la partie (iOS tue parfois sans événement)
  setInterval(() => {
    if (G && !G.over) saveGame();
  }, 15000);
  // pageshow : déclenché au retour depuis le cache iOS (bfcache)
  window.addEventListener('pageshow', (e) => {
    if (e.persisted && G && !G.over) {
      // Page restaurée depuis le cache — reconstruire le DOM du plateau
      buildBoard(); renderBoard(); renderPlayers(); updateTurnUI();
    }
  });
