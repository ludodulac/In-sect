...
function finishTurn(){
  ...
  // Vérifier si l'équipe suivante a au moins un mouvement possible
  // Si toutes ses pièces sont bloquées, passer son tour automatiquement
  let skips = 0;
  while (skips < G.order.length) {
    ...
    if (hasMove) break;
    // Aucun mouvement possible — passer le tour et loguer
    logMove({ color: CCSS[c], text: `<b style="color:${CCSS[c]}">${COLOR_FR[c]}</b> passe son tour (aucun mouvement possible)` });
