...
const STRIPE_URL = "https://buy.stripe.com/4gMfZi3Et2tg5xY8zH1VK00"; // ⚠️ Mettre à jour vers le lien Stripe 4,99€
...
function setInfoPhase(msg) {
  const p = document.getElementById('info-phase');
  const zone = document.getElementById('info-zone');
  if (!p) return;
  if (msg) {
    p.innerHTML = msg; p.classList.add('visible');
    if (zone) zone.classList.add('has-phase');
  } else {
    p.innerHTML = ''; p.classList.remove('visible');
    if (zone) zone.classList.remove('has-phase');
  }
}
...
function closeSPResult() {
  ...
  G.spPaused = false;
  _animating = false;
  // Reprendre le jeu si c'est le tour de l'IA
  if (!isHuman() && !G.over) {
    _aiTimer = setTimeout(() => aiTurn(finishTurn), 600);
  }
}
