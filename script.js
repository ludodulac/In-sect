...
function updateSoundModal() {
  ...
  // Icône bouton jeu
  const btn = document.getElementById('snd-btn-game');
  if (btn) btn.textContent = (_muted && _sfxMuted) ? '🔇' : _muted ? '🎵' : _sfxMuted ? '💥' : '🔊';
}
...
function updatePieceInfo(piece) {
  ...
  const displayImg = piece.type === 'chef' ? 'guepe.png' : SYM[piece.type];

  // Mise à jour de la grande image showcase + nom complet
  ...
  if (showcase && showcaseImg) {
    showcaseImg.src = displayImg;
    showcaseImg.alt = PNAME[piece.type];
    showcase.classList.add('active');
  }
  if (showcaseName) {
    showcaseName.textContent = PFULL[piece.type] || PNAME[piece.type];
    showcaseName.style.color = CCSS[piece.color];
  }

  // Barre d'info texte seulement en dessous (sans image)
  pi.innerHTML = `
    <div class="pi-info">
