...
function isQueenTrapped(color) {
  ...
  // MURS : dépouilles uniquement.
  // Si le fill atteint une case en bord du plateau (r=0/8 ou c=0/8) → pas encerclée.
  // Si le fill reste confiné sans jamais toucher un bord → encerclée.
  ...
  while (queue.length) {
    ...
    // Si on touche un bord du plateau → la Reine peut s'échapper → pas encerclée
  ...
  return true; // fill jamais sorti sur un bord = encerclée
}
