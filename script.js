...
function isQueenTrapped(color) {
  ...
  // Murs = dépouilles + pièces vivantes (la Reine ne peut pas les traverser).
  // Encerclée si le fill ne peut jamais atteindre un bord de l'échiquier.
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) return false; // atteint le bord de l'échiquier → pas encerclée
      const key = `${nr},${nc}`; if (visited.has(key)) continue;
      const cell = G.board[nr][nc];
      if (cell) continue; // toute pièce (vivante ou morte) = mur
      visited.add(key); queue.push([nr,nc]);
    }
  }

  return true; // jamais atteint un bord = encerclée → morte
}
