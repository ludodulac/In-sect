...
// C'est ce qui permet de détecter les combinaisons à 2-3 coups (un piège qui se
// referme au coup d'après, pas seulement une menace immédiate).
...
// coup suivant. Élagage alpha : dès qu'un coup ne peut plus dépasser le meilleur
// déjà trouvé pour d'autres coups candidats, on arrête de l'évaluer plus loin —
// ça libère le budget de calcul économisé pour regarder plus profond ailleurs.
function minimaxScore(mv, myColor, alpha = -Infinity) {
  ...
  for (const opp of opponents) {
    ...
    // 12 réponses adverses plausibles, évaluées superficiellement d'abord
    const oppScored = oppMoves.map(omv => ({ omv, s: scoreAIMove(omv, opp) }));
    oppScored.sort((a,b) => b.s - a.s);
    const candidates = oppScored.slice(0, 12).map(({ omv }) => {
    ...
    candidates.sort((a,b) => a.e - b.e); // les plus dangereuses pour moi en tête

    // Parmi elles, les 2 plus dangereuses sont creusées un coup plus loin (profondeur 3)
    let worstHere = candidates.length ? candidates[0].e : worstForMe;
    for (const cand of candidates.slice(0, 2)) {
      const deep = myBestFollowUp(cand.state2, myColor, 6);
...
function aiLevel3(color){
    ...
    if (chef) {
      ...
      if (escapes.length) {
        ...
        const scored = escapes.map(mv => {
          const s = minimaxScore(mv, color, alpha);
          if (s > alpha) alpha = s;
          return { mv, s };
        });
        scored.sort((a,b) => b.s - a.s);
  ...
  // et permet de creuser plus profond (profondeur 3) sans exploser le temps de calcul.
  const preOrdered = [...pool].sort((a,b) => scoreAIMove(b,color) - scoreAIMove(a,color));

  let alpha = -Infinity;
  const scored = preOrdered.map(mv => {
    // alpha se base sur le score brut du minimax, pas sur le score bonus-ajusté,
    // pour ne pas fausser l'élagage des coups suivants
    const raw = minimaxScore(mv, color, alpha);
