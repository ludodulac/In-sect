...
function doNecro(piece,corpse,toR,toC){
  // Si c'est le SP résurrection, on ne déplace pas le scarabée — on choisit juste une case de placement
...
function aiLevel3(color){
  ...
  // Priorité 3 : utiliser SP résurrection si une pièce forte est disponible
  const necroMoves = all.filter(mv => mv.type === 'necro');
  if (necroMoves.length) {
    const PIECE_VAL = { necromobile:5, reporter:4, assassin:4, araignee:3, coccinelle:3, fourmi:2 };
    const bestNecro = necroMoves.sort((a,b) => (PIECE_VAL[b.target.type]||1) - (PIECE_VAL[a.target.type]||1))[0];
    if ((PIECE_VAL[bestNecro.target.type]||0) >= 3) return bestNecro; // vaut le coup
  }
  // Filtre : la Reine ne se déplace jamais vers une case menacée, sauf dernier recours
  const safe=all.filter(mv=>
    !(mv.piece.type==='chef'&&mv.type==='move'&&countThreatsTo(mv.toR,mv.toC,color)>0)
