...
function renderReplayFrame() {
  ...
  if (cntEl) cntEl.textContent = `Coup ${_moveLog.length - _replayIdx} / ${_moveLog.length}`;
...
function drawReplayBoard(snapshot) {
  ...
  // Nid sacré
  ctx.fillStyle = 'rgba(212,160,23,.25)';
  ctx.fillRect(4 * cs, 4 * cs, cs, cs);
  ctx.font = `${Math.floor(cs * 0.5)}px serif`;
  ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
  ctx.fillText('⊕', 4 * cs + cs / 2, 4 * cs + cs / 2);

  // Pièces
  const SYM_REPLAY = { chef:'🐝', assassin:'🕷️', reporter:'🪰', necromobile:'🪲', diplomate:'🐞', militant:'🐜' };
  const CBG = { yellow:'#B8860B', red:'#8B0000', green:'#1B5E20', blue:'#0D47A1' };
  ...
  for (const p of snapshot.pieces) {
    ...
    ctx.fillStyle = p.dead ? 'rgba(60,60,60,.7)' : (CBG[p.color] || '#333');
    ctx.fill();
    ctx.strokeStyle = p.dead ? '#555' : (CBRD[p.color] || '#fff');
    ctx.lineWidth = 1.5;
    ctx.stroke();
    // Emoji pièce
    ...
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(SYM_REPLAY[p.type] || '?', x + cs/2, y + cs/2);
    ctx.globalAlpha = 1;
  }
}
...
function isQueenTrapped(color) {
  ...
  // Condition 1 : le Scarabée doit être mort (sinon il peut déplacer les dépouilles)
  const hasNecro = pl.pieces.some(p=>!p.dead&&p.type==='necromobile'&&p.color===color);
  if (hasNecro) return false;
  // Flood fill : seules les DÉPOUILLES bloquent (pas les pièces vivantes)
  // Les bords du plateau bloquent naturellement (inB renvoie false)
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) continue; // bord = mur naturel
      const key = `${nr},${nc}`; if (visited.has(key)) continue;
      const cell = G.board[nr][nc];
      if (cell && cell.dead) continue; // dépouille = mur
      // pièce vivante = traversable (ne bloque pas la Reine)
      visited.add(key); queue.push([nr,nc]);
    }
  }
  // Si le flood fill n'a trouvé aucune case libre (autre que la case de la Reine elle-même)
  let freeFound = false;
  for (const key of visited) {
    const [r,c] = key.split(',').map(Number);
    if (r===chef.r && c===chef.c) continue;
    const cell = G.board[r][c];
    if (!cell || !cell.dead) { freeFound = true; break; } // case libre ou pièce vivante = pas encerclée
  }
  return !freeFound;
}
