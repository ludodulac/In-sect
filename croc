...
function startGame() {
  ...
  // Afficher les toggles seulement en mode 3 IA (4 joueurs)
  const togs = document.getElementById('game-toggles');
  const pactBanner = document.getElementById('pact-banner');
  if (togs) togs.style.display = _mode === 3 ? 'flex' : 'none';
  if (pactBanner) pactBanner.style.display = 'none';
  updateToggleUI();
