...
function isQueenTrapped(color) {
  ...
  // Seules les dépouilles sont des murs. Pièces vivantes = traversables.
  // Si le fill atteint l'extérieur du plateau → pas encerclée.
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (!inB(nr,nc)) return false; // atteint l'extérieur → pas encerclée
  ...
  return true; // jamais sorti = encerclée → morte
}
...
function checkStalemates() {
  for (const color of [...G.order]) {
    ...
    console.log(`[encerclement] ${color} : trapped=${trapped}, scarabée vivant=${G.players[color].pieces.some(p=>!p.dead&&p.type==='necromobile')}`);
    if (trapped) {
