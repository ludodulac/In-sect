...
// Décrémente tous les compteurs d'invincibilité actifs (appelé 1x par tour)
function tickAllInvincibles() {
  for (const id in G.spPieces) {
    const sp = G.spPieces[id];
    if (sp.type === 'invincible') {
      sp.turns--;
      if (sp.turns <= 0) {
        // Trouver la pièce pour le message
        let foundPiece = null;
        for (const c of G.order) {
          const p = G.players[c] && G.players[c].pieces.find(p => p.id === parseInt(id));
          if (p) { foundPiece = p; break; }
        }
        delete G.spPieces[id];
        if (foundPiece) {
          const pEl = document.getElementById('p' + id);
          if (pEl) pEl.classList.remove('has-sp');
          showInfoText('⚔️ Invincibilité expirée',
            `La Reine <b style="color:${CCSS[foundPiece.color]}">${CNAME_SHORT[foundPiece.color]}</b> n'est plus invincible !`,
            CCSS[foundPiece.color]);
        }
      }
    }
  }
...
// tickSPTurns — l'invincibilité est gérée globalement par tickAllInvincibles dans finishTurn
