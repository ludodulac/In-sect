...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      return json({ ok: true, code: created.code, secret, status: created.status, version: created.version })
    }
    ...
    if (action === 'join') {
      if (match.status !== 'waiting' || match.guest_secret_hash) {
        return json({ ok: false, error: 'Cette partie a déjà un adversaire.' }, 409)
      }
      const secret = randomSecret()
      ...
      const { data, error } = await db.from('insect_matches')
        ...
        .eq('id', match.id)
        .eq('status', 'waiting')
        .is('guest_secret_hash', null)
        .select('status,version')
        .maybeSingle()
      if (error) throw error
      if (!data) return json({ ok: false, error: 'Un autre joueur vient de rejoindre cette partie.' }, 409)
      return json({ ok: true, secret, status: data.status, version: data.version })
    }
    ...
    if (action === 'get') {
      ...
      const changed = Number(match.version) > since
      return json({
        ok: true,
        role,
        status: match.status,
        version: Number(match.version),
        state: changed ? match.state : null,
      })
    }
