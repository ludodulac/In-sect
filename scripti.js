...
function isQueenTrapped(color) {
  ...
  // Flood-fill depuis la Reine : seules les dépouilles et les bords sont des murs.
  // Les pièces vivantes (alliées OU ennemies) sont traversables.
  // Si le fill peut atteindre une case vide qui n'est pas bloquée de tous côtés → pas piégée.
  const visited = new Set();
  const start = `${chef.r},${chef.c}`;
  visited.add(start);
  const queue = [[chef.r, chef.c]];
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      visited.add(key);
      // Si cette case vide (ou pièce vivante) est accessible depuis l'extérieur → pas piégée
      // On vérifie si depuis cette case on peut atteindre le bord du plateau
      // en fait : si visited contient une case en bord de plateau → pas piégée
      if (nr===0||nr===8||nc===0||nc===8) return false;
      queue.push([nr,nc]);
    }
  }
  return true; // jamais atteint le bord → encerclée
}
