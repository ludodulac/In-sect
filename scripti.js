...
function isQueenTrapped(color) {
  ...
  // Tout le reste (vide, pièces vivantes) est traversable.
  // Si la zone atteint un bord → ouverte → pas piégée.
  // Si la zone reste confinée sans toucher un bord → encerclée.
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      visited.add(key);
      if (nr===0||nr===8||nc===0||nc===8) return false; // zone ouverte
      queue.push([nr,nc]);
    }
  }
