...
function updatePieceInfo(piece) {
  ...
  // Préparer le contenu du modal d'explication
  const sp = G.spPieces && G.spPieces[piece.id];
  const spTxt = sp ? `<br><br><span style="color:#C080FF;">✨ <b>Super pouvoir actif</b> : ${SP_DESC[sp.type].split('!')[0]}${sp.type==='invincible'?' ('+sp.turns+' coups restants)':''}.</span>` : '';
  _pieceExplainTitle = (SYM[piece.type] ? '' : '') + (PFULL[piece.type] || PNAME[piece.type]);
  _pieceExplainBody  = (PDESC[piece.type] || '') + spTxt;
  // Afficher le bouton
  const btn = document.getElementById('btn-piece-explain');
  if (btn) { btn.style.display = ''; btn.textContent = `📖 ${PFULL[piece.type] || PNAME[piece.type]}`; }
}
...
function showInfoText(name, desc, color) {
  ...
  // Si c'est le Nid Sacré, préparer un modal dédié
  const btn = document.getElementById('btn-piece-explain');
  if (desc && desc.length > 40) {
    _pieceExplainTitle = name;
    _pieceExplainBody  = desc;
    if (btn) { btn.style.display = ''; btn.textContent = `📖 ${name}`; }
  } else {
...
function setInfoDesc(html) {
  // Plus d'affichage inline — contenu stocké pour le modal uniquement
  // Garde la compatibilité avec les appels existants
  if (html) {
    _pieceExplainBody = html;
  }
}
...
function doKill(piece,victim,toR,toC){
  ...
  animMove(piece,toR,toC).then(()=>{
    checkSPCellCapture(piece,toR,toC); // SP possible sur case de kill aussi
    tickSPTurns(piece);
    const sp = G.spPieces[piece.id];
  ...
  if(type==='move'){
    ...
    animMove(piece,toR,toC).then(()=>{
      if(piece.type==='reporter'){
        const rts=getRepAdj(piece);
        if(rts.length>0){
          // Choisir la direction qui tue le plus de pièces, en priorisant la Reine ennemie
    ...
    // Mouche : se déplacer puis tirer dans la bonne direction
    logMoveAction(piece,'move',null);
    animMove(piece,toR,toC).then(()=>{
...
function checkSPCellCapture(piece, r, c) {
  ...
  if (spType) {
    ...
    const pEl = document.getElementById('p' + piece.id);
    if (pEl) pEl.classList.add('has-sp');
    // Logger l'obtention du super pouvoir
    const pc = CCSS[piece.color];
    const pn = `<b style="color:${pc}">${PNAME_SHORT[piece.type]} ${COLOR_FR[piece.color]}</b>`;
