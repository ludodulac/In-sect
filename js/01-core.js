...
function _onlineRuntime() {
  return (typeof window !== 'undefined' && window.INSECT_MP_RUNTIME) ? window.INSECT_MP_RUNTIME : null;
}

function saveGame() {
  const rt = _onlineRuntime();
  if (rt && rt.isOnline()) return;
  if (!G || G.over || !G.board) return;
  try {
    const snap = {
      G: JSON.stringify(G), mode: _mode, aiLevel: _aiLevel, selColor: _selColor,
      optSP: _optSP, uid: _uid, turns: _gameTurns, caps: _gameCaps,
      moveLog: _moveLog, ts: Date.now()
    };
    localStorage.setItem(LS.SAVE, JSON.stringify(snap));
  } catch(e) {}
}
function clearSave() { try { localStorage.removeItem(LS.SAVE); } catch(e) {} }
function hasSave() { try { return !!localStorage.getItem(LS.SAVE); } catch(e) { return false; } }
function loadSave() {
  try {
    const raw = localStorage.getItem(LS.SAVE); if (!raw) return false;
    const snap = JSON.parse(raw);
    _mode = snap.mode; _aiLevel = snap.aiLevel; _selColor = snap.selColor; _optSP = snap.optSP;
    _uid = snap.uid; _gameTurns = snap.turns; _gameCaps = snap.caps; _moveLog = snap.moveLog || [];
    G = JSON.parse(snap.G);
    G.board = Array.from({length:9}, ()=>Array(9).fill(null));
    for (const c of G.order) {
      const pl = G.players[c]; if (!pl) continue;
      for (const p of pl.pieces) if (!p.dead) G.board[p.r][p.c] = p;
    }
    return true;
  } catch(e) { clearSave(); return false; }
}

const CROWN_SVG = `<svg width="12" height="10" viewBox="0 0 12 10" fill="none" xmlns="http://www.w3.org/2000/svg" style="display:inline-block;vertical-align:middle;margin-right:4px"><path d="M1 9h10M1 9L0 3l3 2.5L6 1l3 4.5L9 5.5L12 3l-1 6H1z" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" fill="none"/></svg>`;
...
function updateStatusBar() {
  ...
  if (scoreBar) {
    ...
    const w = parseInt(lsGet(wKey,'0'),10), l = parseInt(lsGet(lKey,'0'),10);
    if (wLabel) wLabel.textContent = `▲ ${w}`;
    if (lLabel) lLabel.textContent = `▼ ${l}`;
    scoreBar.style.display = 'flex';
  }
  if (!counter || !badge) return;
  counter.textContent = '♾️ Parties illimitées'; counter.className = 'status-counter';
  badge.innerHTML = CROWN_SVG + 'Partager le jeu'; badge.className = 'status-badge unlocked';
...
async function doShare() {
  ...
  const scoreMsg = wins > 0 ? `🏆 ${wins} victoire${wins>1?'s':''} en ${wins+losses} parties — tu peux faire mieux ?` : '⚔️ 4 colonies en guerre. 1 seule survivra.';
  const shareData = {
  ...
  if (!navigator.share) { openShareDesktopModal(shareData); return; }
  const bgmWasPlaying = _bgm && !_bgm.paused; if (bgmWasPlaying) stopBGM();
  try { await navigator.share(shareData); toast('🤝 Merci pour le partage !'); }
  catch (err) { if (err.name !== 'AbortError') toast('Partage annulé.'); }
  finally { if (bgmWasPlaying) setTimeout(() => playBGM(),300); }
}

function openShareDesktopModal(shareData) {
  const url = shareData.url, txt = encodeURIComponent(shareData.text);
  const existing = document.getElementById('modal-share-desktop'); if (existing) existing.remove();
...
function resetScore() {
  ...
  for (const m of modes) for (const l of levels) {
    lsSet(`insect_wins_${m}ia_${l}`,'0'); lsSet(`insect_losses_${m}ia_${l}`,'0');
  }
  updateStatusBar();
}
...
let _moveLog = [];
function logMove(entry) { _moveLog.unshift(entry); if (_moveLog.length > 20) _moveLog.length = 20; }
function openMoveLogModal() {
  const el = document.getElementById('movelog-body'); if (!el) return;
  if (!_moveLog.length) {
  ...
  el.scrollTop = 0; showModal('movelog');
}
function openReplayModal() { openMoveLogModal(); }
...
function logMoveAction(piece, type, target) {
  ...
  let text = '';
  if (type === 'move') text = `${pn} se déplace`;
  else if (type === 'kill') {
    const tc = CCSS[target.color], tn = `<b style="color:${tc}">${PNAME_SHORT[target.type]} ${COLOR_FR[target.color]}</b>`;
    text = `${pn} tue ${tn}`;
  } else if (type === 'dipl') {
    const tc = CCSS[target.color], tn = `<b style="color:${tc}">${PNAME_SHORT[target.type]} ${COLOR_FR[target.color]}</b>`;
    text = `${pn} déplace ${tn} vers une case libre`;
  } else if (type === 'necro') text = `${pn} repositionne une dépouille 💀`;
  ...
  } else if (type === 'place-corpse') text = `${pn} place une dépouille 💀 sur une case choisie`;
  else text = `${pn} joue`;
  logMove({ color: CCSS[piece.color], text, isKill: type === 'kill' });
}

function _scoreKey(type) { return `insect_${type}_${_mode}ia_${_aiLevel}`; }
...
function tryStartGame() { startGame(); }
...
const isHuman = () => {
  const rt = _onlineRuntime();
  if (rt && rt.isOnline()) return rt.allowsBoardInput();
  return G.players[cur()] && G.players[cur()].human;
};
