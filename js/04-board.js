...
function startGame() {
  ...
  if (!isHuman()) _aiTimer = setTimeout(() => aiTurn(finishTurn), 900);
  sfxStart();
}

/* ═══════════════════════════════════════════
   BOARD — Perspective d'affichage
   Les coordonnées logiques de G ne changent jamais.
   ═══════════════════════════════════════════ */
function _boardRuntime() {
  return (typeof window !== 'undefined' && window.INSECT_MP_RUNTIME) ? window.INSECT_MP_RUNTIME : null;
}
function logicalToView(r,c) {
  const rt = _boardRuntime();
  return rt && rt.isOnline() ? rt.mapLogicalToView(r,c) : {r,c};
}
function _positionStyle(r,c,cs,offset=0) {
  const view = logicalToView(r,c);
  return { left:view.c*cs+offset, top:view.r*cs+offset };
}
...
function drawPiece(cv, color, sym, dead, selected) {
  ...
  const imgX = (w - imgSize) / 2, imgY = (h - imgSize) / 2;
  if (dead) ctx.globalAlpha = 0.35;
  if (img.complete && img.naturalWidth > 0) ctx.drawImage(img, imgX, imgY, imgSize, imgSize);
  else img.onload = () => { try { drawPiece(cv, color, sym, dead, selected); } catch(e){} };
  ctx.globalAlpha = 1;
}
...
function buildBoard() {
  ...
  for (let r = 0; r < 9; r++) for (let c = 0; c < 9; c++) {
    ...
    const pos = _positionStyle(r,c,cs);
    cell.id = `cell-${r}-${c}`;
    cell.className = 'cell ' + (isLab ? 'lab' : ((r+c)%2===0?'l':'d'));
    cell.style.cssText = `left:${pos.left}px;top:${pos.top}px;width:${cs}px;height:${cs}px`;
    if (isLab) {
  ...
  const psize = cs, offset = 0;
  for (const c of G.order) {
    for (const p of G.players[c].pieces) {
      const pos = _positionStyle(p.r,p.c,cs,offset);
      const pe = document.createElement('div');
      pe.id = 'p'+p.id; pe.className = `piece ${p.color}`;
      pe.style.cssText = `left:${pos.left}px;top:${pos.top}px;width:${psize}px;height:${psize}px;--anim:${ANIM_MS}ms`;
      pe.style.transform = '';
...
function renderBoard() {
  ...
  if (G.phase === 'nid-bonus' && G._nidBonusMoves) for (const m of G._nidBonusMoves) hl[`${m.r},${m.c}`] = 'vm';
  ...
  if (G.phase === 'place-corpse' || G.phase === 'place-necro' || G.phase === 'place-dipl' || G.phase === 'place-resurrect') {
    for (let r = 0; r < 9; r++) for (let c = 0; c < 9; c++) if (!G.board[r][c] || G.board[r][c].dead) hl[`${r},${c}`] = 'vplace';
  }
  for (let r = 0; r < 9; r++) for (let c = 0; c < 9; c++) {
  ...
  for (const p of allPieces) {
    ...
    if (!onBoard) continue;
    const psize = cs, offset = 0, pos = _positionStyle(p.r,p.c,cs,offset);
    if (!pe.classList.contains('moving')) { pe.style.left = pos.left+'px'; pe.style.top = pos.top+'px'; }
    pe.style.transform = '';
    const isSel = G.sel && G.sel.id === p.id;
    const hasSP = G.spPieces && G.spPieces[p.id];
    const isGhost = hasSP && hasSP.ghost;
    ...
    else if (isGhost) pe.style.filter = `drop-shadow(0 0 6px #C080FF) drop-shadow(0 0 3px #C080FF)`;
...
function updateTurnUI() {
  ...
  const rt = _boardRuntime();
  if (ttxt) {
    const onlineLabel = rt && rt.isOnline() ? rt.getTurnLabel(c,CNAME[c]) : null;
    ttxt.textContent = onlineLabel || `Tour ${CNAME[c]}${pl.human?' — À vous !':''}`;
    ttxt.style.color = CCSS[c];
  }
  const modeBadge = document.getElementById('mode-badge');
  if (modeBadge) {
    if (rt && rt.isOnline()) {
      const snap = rt.snapshot();
      modeBadge.textContent = `${rt.identityLabel()} · EN LIGNE${snap.code?' · '+snap.code:''}`;
    } else {
...
function updatePieceInfo(piece) {
  ...
  } else _setExplainBtn(pieceName,pieceName,PDESC[piece.type] || '');
}
function resetTopbar() { const btn=document.getElementById('btn-piece-explain'); if(btn)btn.style.display='none'; }
function openPieceExplainModal() {
  const title=document.getElementById('piece-explain-title'), body=document.getElementById('piece-explain-body');
...
function showInfoText(name,desc,color) {
  if (desc && desc.length > 30) _setExplainBtn(name,name,desc);
  else resetTopbar();
}
function setInfoDesc(html) { if(html)_pieceExplainBody=html; }
...
function setInfoPhase(msg) {
  ...
  if(msg){cap.innerHTML=msg;cap.classList.add('visible');}
  else{cap.classList.remove('visible');cap.innerHTML='';}
}
...
function animMove(piece,tr,tc) {
  return new Promise(res => {
    ...
    const cs=getCellSize(), pos=_positionStyle(tr,tc,cs,0);
    pe.style.left=pos.left+'px';pe.style.top=pos.top+'px';
    sfxMove();
    setTimeout(()=>{pe.classList.remove('moving');movePiece(piece,tr,tc);_animating=false;res();},ANIM_MS+30);
  });
}
function boardShake() { const f=document.getElementById('bframe');if(!f)return;f.classList.add('shake');setTimeout(()=>f.classList.remove('shake'),400); }
...
function getPiecePos(piece) {
  ...
  const br=be.getBoundingClientRect(),cs=getCellSize(),view=logicalToView(piece.r,piece.c);
  return{x:br.left+view.c*cs+cs/2,y:br.top+view.r*cs+cs/2};
}
