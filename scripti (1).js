...
function isQueenTrapped(color) {
  ...
  // Flood-fill depuis la Reine. Seul mur : les dépouilles.
  // Les pièces vivantes sont traversables (elles font partie de la zone).
  // Une sortie = case de bord VIDE (sans pièce vivante ni dépouille).
  // Si la zone contient une case de bord vide → ouverte → pas piégée.
  // Si la zone reste confinée sans case de bord vide → encerclée.
  ...
  while (queue.length) {
    ...
    for (const [dr,dc] of DIRS8) {
      ...
      if (cell && cell.dead) continue; // dépouille = mur, bloque
      visited.add(key);
      // Sortie = case de bord sans occupation (vide)
      if ((nr===0||nr===8||nc===0||nc===8) && !cell) return false;
      queue.push([nr,nc]);
    }
  }
  return true; // zone fermée → encerclée
}
